...
# ### Run pipeline
# network_construction_time, cellml_model_generation_time, num_vessels = run_image_to_model(target_input_image_path, target_output_image_path, resources_path, ilastik_path, model_path,
...
for sv in sub_volumes:
    ...
    network_time, cellml_time, num_vessels, t_gen_files, t_parse, t_resolve, t_flatten, t_print, t_analyser = run_image_to_model(
...
with open(csv_filename, mode='w', newline='') as csv_file:
    ...
    for v, net_t, cell_t, t_gen, t_par, t_res, t_fla, t_pri, t_ana in zip(
        ...
        writer.writerow([v, net_t, cell_t, t_gen, t_par, t_res, t_fla, t_pri, t_ana])
...
# 2. NEW: Plot the 6 Micro-Timers (Dashed lines with distinct markers)
...
# ===============================================
# FIGURE 2: Extrapolated Exponential Fit Plot
# ===============================================
...
try:
    print("\nFitting exponential curves for extrapolation...")
    
    # Fit the main times
    popt_net, _ = curve_fit(exp_func, sorted_vessels, sorted_network_times, p0=(1, 0.01), maxfev=10000)
    popt_cellml, _ = curve_fit(exp_func, sorted_vessels, sorted_cellml_times, p0=(1, 0.01), maxfev=10000)

    # NEW: Safety fallback to prevent crashes if micro-timers are perfectly flat (0.0s)
    def robust_fit(y_data):
        try:
            popt, _ = curve_fit(exp_func, sorted_vessels, y_data, p0=(1, 0.01), maxfev=10000)
            return popt
        except:
            return [0, 0] # Fallback to a flat line if it fails to converge
    ...
    # Calculate R-squared values for all 8 lines
    r2_net = calculate_r_squared(sorted_network_times, exp_func(sorted_vessels, *popt_net))
    r2_cellml = calculate_r_squared(sorted_cellml_times, exp_func(sorted_vessels, *popt_cellml))
    r2_gen = calculate_r_squared(sorted_t_gen_files, exp_func(sorted_vessels, *popt_gen))
    r2_parse = calculate_r_squared(sorted_t_parse, exp_func(sorted_vessels, *popt_parse))
    r2_resolve = calculate_r_squared(sorted_t_resolve, exp_func(sorted_vessels, *popt_resolve))
    r2_flatten = calculate_r_squared(sorted_t_flatten, exp_func(sorted_vessels, *popt_flatten))
    r2_print = calculate_r_squared(sorted_t_print, exp_func(sorted_vessels, *popt_print))
    r2_analyser = calculate_r_squared(sorted_t_analyser, exp_func(sorted_vessels, *popt_analyser))
    ...
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_net), color='blue', linestyle='--', linewidth=2, alpha=0.8, label=f'Network Construction ($R^2$={r2_net:.3f})')
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_cellml), color='red', linestyle='--', linewidth=2, alpha=0.8, label=f'generate_with_new_architecture() ($R^2$={r2_cellml:.3f})')
    
    # Plot the micro-timer extrapolated curves
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_gen), linestyle=':', color='green', linewidth=2, label=f'generate_files() funcs ($R^2$={r2_gen:.3f})')
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_parse), linestyle=':', color='orange', linewidth=2, label=f'parse_model() ($R^2$={r2_parse:.3f})')
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_resolve), linestyle=':', color='purple', linewidth=2, label=f'resolve_imports() ($R^2$={r2_resolve:.3f})')
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_flatten), linestyle=':', color='brown', linewidth=2, label=f'flatten_model() ($R^2$={r2_flatten:.3f})')
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_print), linestyle=':', color='pink', linewidth=2, label=f'print_model() ($R^2$={r2_print:.3f})')
    plt.plot(extrapolated_vessels, exp_func(extrapolated_vessels, *popt_analyser), linestyle=':', color='gray', linewidth=2, label=f'Analyser() Methods ($R^2$={r2_analyser:.3f})')
    ...
    max_y_values = [
        np.max(exp_func(extrapolated_vessels, *popt_net)),
        np.max(exp_func(extrapolated_vessels, *popt_cellml)),
        np.max(exp_func(extrapolated_vessels, *popt_gen)),
        np.max(exp_func(extrapolated_vessels, *popt_parse)),
        np.max(exp_func(extrapolated_vessels, *popt_resolve)),
        np.max(exp_func(extrapolated_vessels, *popt_flatten)),
        np.max(exp_func(extrapolated_vessels, *popt_print)),
        np.max(exp_func(extrapolated_vessels, *popt_analyser))
    ]
    ...
    plt.savefig("extrapolated_runtime_plot.png", dpi=300, bbox_inches='tight')
    print("Extrapolated plot successfully saved as 'extrapolated_runtime_plot.png'")
